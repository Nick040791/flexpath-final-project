...
jest.mock("../components/Breadcrumbs", () => ({ __esModule: true, default: (props) => <breadcrumbs {...props} /> }));
...
describe("MyPartsPage", () => {
    ...
    test("requires authentication", () => {
        ...
        expect(renderer.root.findAll(
            (node) => node.children?.includes("Please log in to manage your parts.")
        )).not.toHaveLength(0);
    });

    test("loads, renders its breadcrumb, creates, and deletes owned parts", async () => {
        ...
        expect(renderer.root.findByType("breadcrumbs").props.items).toEqual([
            { label: "Home", to: "/" },
            { label: "My Parts" },
        ]);
