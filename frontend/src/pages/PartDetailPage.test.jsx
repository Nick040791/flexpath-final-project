...
describe("PartDetailPage", () => {
    ...
    test("loads a manageable part, renders its breadcrumb, and supports update/delete", async () => {
        const part = {
            id: 7,
            name: "GPU",
            username: "nick",
        ...
        expect(breadcrumbItems[breadcrumbItems.length - 1].children).toEqual(["GPU"]);

        const buttons = renderer.root.findAllByType("button");
        act(() => buttons.find((button) => button.children.includes("Edit")).props.onClick());

        await act(async () => {
            await renderer.root.findByType("part-form").props.onSubmit({ name: "Updated" });
        });
